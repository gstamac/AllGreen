...
describe("AllGreen.RunnerReporter", function () {
    ...
    it("Should be read", function () {
        expect(_this.runnerReporter.isReady()).toBeTruthy();
    });

    it("Can be reset", function () {
        _this.runnerReporter.started();
        _this.runnerReporter.specUpdated({ id: 123, name: 'test 1', suite: _this.suite1, status: 2 /* Passed */ });
    ...
    it("Displays spec status", function () {
        _this.runnerReporter.specUpdated({ id: 123, name: 'test 1', suite: _this.suite1, status: 2 /* Passed */ });
        expect(_this.suite1.status).toBe(2 /* Passed */);
    ...
    it("Displays failed spec steps", function () {
        var steps = [
            { message: 'log message' },
            { message: 'expected something and succeded', status: 2 /* Passed */ },
            { message: 'expected something and failed', status: 3 /* Failed */, trace: 'trace 1\ntrace 2' }
        ];
        _this.runnerReporter.specUpdated({ id: 123, name: 'test 1', suite: _this.suite1, status: 3 /* Failed */, steps: steps });
    ...
    it("Updates spec status", function () {
        _this.runnerReporter.specUpdated({ id: 123, name: 'test 1', suite: _this.suite1, status: 2 /* Passed */ });
        _this.runnerReporter.specUpdated({ id: 123, name: 'test 1', suite: _this.suite1, status: 3 /* Failed */ });
        expect(_this.suite1.status).toBe(3 /* Failed */);
        expect($('#spec-results > #suite-444 > .suite-content > #spec-123')).toBeSpecElement('test 1', 'failed');
    ...
    it("Updates suite status", function () {
        _this.runnerReporter.specUpdated({ id: 123, name: 'test 1', suite: _this.suite1, status: 2 /* Passed */ });
        _this.runnerReporter.specUpdated({ id: 323, name: 'test 2', suite: _this.suite1, status: 3 /* Failed */ });
        expect(_this.suite1.status).toBe(3 /* Failed */);
        expect($('#spec-results > #suite-444 > .suite-content > #spec-123')).toBeSpecElement('test 1', 'passed');
    ...
    it("Supports nested suites", function () {
        var suite2 = {
            id: 744, name: 'suite 2', parentSuite: _this.suite1, status: 0 /* Undefined */
        };
        _this.runnerReporter.specUpdated({ id: 123, name: 'test 1', suite: suite2, status: 2 /* Passed */ });
        _this.runnerReporter.specUpdated({ id: 323, name: 'test 2', suite: suite2, status: 3 /* Failed */ });
        expect(_this.suite1.status).toBe(3 /* Failed */);
        expect(suite2.status).toBe(3 /* Failed */);
    ...
    it("Shows final suite status", function () {
        _this.runnerReporter.specUpdated({ id: 123, name: 'test 1', suite: _this.suite1, status: 1 /* Running */ });
        _this.runnerReporter.specUpdated({ id: 223, name: 'test 2', suite: _this.suite1, status: 1 /* Running */ });
        expect(_this.suite1.status).toBe(1 /* Running */);
        ...
        expect($('#spec-results > #suite-444')).toBeSuiteElement('suite 1', 'running');
        expect($('#spec-results')).toHaveClass('running');
        _this.runnerReporter.specUpdated({ id: 223, name: 'test 2', suite: _this.suite1, status: 3 /* Failed */ });
        expect(_this.suite1.status).toBe(3 /* Failed */);
        expect($('#spec-results > #suite-444')).toBeSuiteElement('suite 1', 'failed');
        expect($('#spec-results')).toHaveClass('failed');
    });
